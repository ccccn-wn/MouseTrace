...
version 0.2
更新：引入坐标变化，优化模型
模型输出：本次坐标点=上一次坐标点+0.3*上一次坐标点和上上次坐标点的差值
'''
...
#函数作用：输出卡尔曼滤波计算值
#                   模型坐标            测量噪声        实际坐标   采样点个数    上一次测量值
def kalmanfilterCal(prex,prey,event,nosiePx,nosiePy,pointx,pointy,count,last_mesx,last_mesy):
    global pk
    global kk
    global pk_pre
    global last_pk
    ...
    if count==1:
        ...
        #先验估计    
        x_hat=prex[count-2]+v*(last_mesx-prex[count-2])
        y_hat=prey[count-2]+v*(last_mesy-prey[count-2])
...
def update1(event,pointx,pointy,mesx,mesy,prex,prey,count):
    ...
    kalmanfilterCal(prex,prey,event,nosiePx,nosiePy,pointx,pointy,count,last_mesx,last_mesy)
    last_mesy=event.y+nosiePy
    last_mesx=event.x+nosiePx
...
window.title("Kalman Filter-Version0.2")
cv.pack()
